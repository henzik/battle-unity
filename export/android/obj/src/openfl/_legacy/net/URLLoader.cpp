...
Void URLLoader_obj::__pollData( ){
{
		...
		HX_STACK_LINE(415)
		Dynamic tmp = ::openfl::_legacy::net::URLLoader_obj::eventsQueue->__Field(HX_HCSTRING("shift","\x82","\xec","\x22","\x7c"), hx::paccDynamic )();		HX_STACK_VAR(tmp,"tmp");
		HX_STACK_LINE(415)
		Dynamic evt = tmp;		HX_STACK_VAR(evt,"evt");
		HX_STACK_LINE(416)
		bool tmp1 = (evt != null());		HX_STACK_VAR(tmp1,"tmp1");
		HX_STACK_LINE(416)
		if ((tmp1)){
			HX_STACK_LINE(417)
			::openfl::_legacy::events::Event tmp2 = evt->__Field(HX_HCSTRING("event","\x1a","\xc8","\xc4","\x75"), hx::paccDynamic );		HX_STACK_VAR(tmp2,"tmp2");
			HX_STACK_LINE(417)
			evt->__Field(HX_HCSTRING("loader","\x13","\x48","\x6f","\x58"), hx::paccDynamic )->__Field(HX_HCSTRING("dispatchEvent","\x00","\xc7","\x64","\xc6"), hx::paccDynamic )(tmp2);
		}
	}
return null();
}
